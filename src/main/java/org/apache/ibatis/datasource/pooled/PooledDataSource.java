...
public class PooledDataSource implements DataSource {
   ...
   * 一个 PooledDataSource 持有一个独立的PoolState，管理此 PooledDataSource 中的所有连接状态
   */
  private final PoolState state = new PoolState(this);
  /**
   * UnpooledDataSource 对象,重用 UnpooledDataSource 的代码.
   * 获取真正连接的逻辑，还是在 UnpooledDataSource 中实现。
   */
  ...
  /**
   * 获取连接的等待时间，如果超出等待时间后，就会重新获取另一个连接
   */
    ...
    while (conn == null) {
      synchronized (state) {
        ...
        if (!state.idleConnections.isEmpty()) {
          // 1.1 通过移除的方式，获得首个空闲的连接
          conn = state.idleConnections.remove(0);
          if (log.isDebugEnabled()) {
            log.debug("Checked out connection " + conn.getRealHashCode() + " from pool.");
          }
        } else {
          // 2. 如果连接为空，是否达到最大活跃连接数
          if (state.activeConnections.size() < poolMaximumActiveConnections) {
            // 2.1 如果激活的连接数小于 poolMaximumActiveConnections
            conn = new PooledConnection(dataSource.getConnection(), this);
            if (log.isDebugEnabled()) {
              log.debug("Created connection " + conn.getRealHashCode() + ".");
            }
          } else {
            // 3. 连接为空，并且达到最大活跃连接数，那么获得首个激活的 PooledConnection 对象，验证是否已经超时
            ...
            if (longestCheckoutTime > poolMaximumCheckoutTime) {
              ...
              // 3.1.1 对连接超时的时间的统计，并将连接从活跃的连接中进行移除
              ...
              // 3.1.2 创建新的 PooledConnection 连接对象
